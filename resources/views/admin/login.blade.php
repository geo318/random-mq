<x-layout>
    <div class="flex min-h-full items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div class="w-full max-w-md space-y-8 p-5 rounded-xl bg-gray-100">
          <div>
            <h2 class="mt-6 text-center text-3xl font-bold tracking-tight text-gray-900">{{ __('main.sign_in_to_your_account') }}</h2>
          </div>
          <form class="mt-8 space-y-6" action="{{ route('login',app()->getLocale()) }}" method="POST">
            ...
            <div class="space-y-px rounded-md shadow-sm">
              <div class="mb-5">
                <label for="email" class="sr-only">{{ __('main.email_address') }}</label>
